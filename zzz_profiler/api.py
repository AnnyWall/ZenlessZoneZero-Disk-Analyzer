...
def get_user_profile(uid: str):
    ...
    if uid in _profile_cache:
        cached_data, cache_time = _profile_cache[uid]
    ...
    try:
        ...
        if raw_data.agents:
            for agent in raw_data.agents:
                agents_data.append(process_agent_data(agent))
        
        final_response = {"player": player_data, "agents": agents_data}
        _profile_cache[uid] = (final_response, current_time)
        
        expired_keys = [k for k, (_, t) in _profile_cache.items() if current_time - t > _cache_ttl]
